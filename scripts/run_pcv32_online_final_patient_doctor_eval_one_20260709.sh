...
echo "max_turns=$MAX_TURNS max_groups=$MAX_GROUPS max_profiles=$MAX_PROFILES max_per_slot=$MAX_PER_SLOT" | tee -a "$MAIN_LOG"
...
generate_patient_cache_for_probe() {
  ...
  "$PY" scripts/filter_patient_realizer_requests_by_cache_v1.py \
...
PY
  ...
  "$PY" scripts/merge_patient_realizer_caches_v1.py \
    --cache-path "$GLOBAL_CACHE" \
