...
ENV_FILE="${1:-${CLOSED_ENV_FILE:-.env}}"
MODEL="${2:-gpt-4.1-mini}"
RUN_DIR="${3:-outputs_patient_realizer_rubric_final}"
...
REPORT_DIR="$RUN_DIR/closed_llm_rubric_judge_report_${MODEL_SAFE}_full"
SHARD_DIR="$RUN_DIR/rubric_sharded_${MODEL_SAFE}_$(date +%Y%m%d_%H%M%S)"
...
"$PY" scripts/prepare_and_merge_rubric_shards.py \
  --mode prepare \
...
"$PY" scripts/summarize_patient_realizer_rubric_outputs.py \
  --request-path "$REQUEST_PATH" \
  --output-path "$OUTPUT_PATH" \
